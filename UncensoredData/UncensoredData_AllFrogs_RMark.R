...
# all frogs (information reported in the appendix of the following paper)
#
# Reference:
...
llf.inp <- convert.inp("UncensoredData_AllFrogs_InputFile.inp",group.df=data.frame(site=c('CH','MA','RN','WHL','WHM','WHU')),use.comments=TRUE)
summary(llf.inp)
...
results.allfrogs.nocensor <- models.comb()

results.allfrogs.nocensor$model.table[,c('model','npar','AICc','DeltaAICc','weight')]
...
(monphi <- results.allfrogs.nocensor[[5]]$results$real[1,1])
(monphi.se <- results.allfrogs.nocensor[[5]]$results$real[1,2])
(annphi <- monphi^12)
...
results.allfrogs.nocensor[[6]]$results$real               #phi = 0.81
results.allfrogs.nocensor[[6]]$results$real[1,2]/results.allfrogs.nocensor[[6]]$results$real[1,1]*100 #CV = 1.38%

#Clean up MARK files:
rm(results.allfrogs.nocensor)
cleanup(ask=F)
list.files()
